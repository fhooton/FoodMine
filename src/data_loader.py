...
def load_raw_data(food, load):
    ...
    if report:
        ...
        report_stat(f'Number of papers reviewed {food}: ' + str(len(food_scoring[food_scoring.is_useful.notnull()])),
                    f'num_reviewed_papers_{food}.txt')
        report_stat(f'Number of unique papers {food}: ' + str(len(food_data['PMID'].drop_duplicates())),
...
def partition_raw_data(food_data, food_scoring):
    ...
    food_data_both = food_data_q.iloc[
        [idx for idx, row in food_data_q.fillna('placeholder').iterrows() if row['chemical'] in unq_chems]]

    # Remove occurrences of overlaping chemicals from the unquantified garlic data
    q_chems = list(set(food_data_q['chemical'].tolist()))
    food_data_dnq = food_data_dnq.iloc[
        [idx for idx, row in food_data_dnq.fillna('placeholder').iterrows() if row['chemical'] not in q_chems]]
...
def load_foodb_data(food, load):
    ...
    quant_foodb_food = foodb_food[foodb_food.standard_content.notnull()][
        ['chem_id', 'chem_id_f', 'orig_source_id', 'name', 'standard_content']].drop_duplicates()
    ...
    unquant_foodb_food = foodb_food[foodb_food.standard_content.isnull()][
        ['chem_id', 'chem_id_f', 'orig_source_id', 'name', 'standard_content']].reset_index()
    ...
    if report:
        ...
        report_stat(f'QFDB size {food}: ' + str(len(quant_foodb_food.chem_id.drop_duplicates())),
                    f'qfdb_size_{food}.txt')
        report_stat(f'UQFDB size {food}: ' + str(len(unquant_foodb_food.chem_id.drop_duplicates())),
...
def load_ctd():
    ...
    hdata.columns = ['ChemicalName', 'ChemicalID', 'CasRN', 'DiseaseName', 'DiseaseID', 'DirectEvidence',
...
def load_health():
    ...
    de_health = pd.DataFrame(
        hdata[hdata.pubchem_id.notnull() & hdata.DirectEvidence.notnull()][
            ['pubchem_id', 'DirectEvidence', 'ChemicalName']]
